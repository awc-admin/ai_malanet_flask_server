...
class BatchJobManager:
    ...
    def create_job(self, job_id: str, 
                   ...
                   use_url: bool, 
                   classifier_params_path: str = None,
                   hitax_type: str = None,
        ...
        _env_settings=[
                EnvironmentSetting(name='API_INSTANCE_NAME', value=api_config.API_INSTANCE_NAME), # cp
                EnvironmentSetting(name='JOB_CONTAINER_SAS', value=input_container_sas), 
                EnvironmentSetting(name='JOB_USE_URL', value=str(use_url)),
                EnvironmentSetting(name='STORAGE_ACCOUNT_NAME', value=api_config.STORAGE_ACCOUNT_NAME), # australianwildlifedia830
                EnvironmentSetting(name='STORAGE_CONTAINER_API', value=api_config.STORAGE_CONTAINER_API), # batch-api
                EnvironmentSetting(name='DETECTOR_REL_PATH', value=detector_path), # models/megadetector_v5/md_v5a.0.0.pt
        ...
        job = JobAddParameter(
            ...
            common_environment_settings=_env_settings
        )

        self.batch_client.job.add(job)

    def submit_tasks(self, job_id: str, num_images: int, hitax_type: str) -> Tuple[int, list]:
        """
        ...
        Args:
            ...
            hitax_type: type of HiTax classifier to use (e.g., ['off', 'label rollup', 'hitax classifier']
        Returns:
        ...
        container_sas_token = generate_container_sas(
            account_name=api_config.STORAGE_ACCOUNT_NAME, #australianwildlifedia830
            container_name=api_config.STORAGE_CONTAINER_API, # batch-api
            account_key=api_config.STORAGE_ACCOUNT_KEY,
            permission=permissions,
            expiry=datetime.now(timezone.utc) + timedelta(hours=access_duration_hrs))
        ...
        for task_id in range(num_tasks):
            ...
            stderr_destination = OutputFileDestination(
                container=OutputFileBlobContainerDestination(
                    ...
                    # api_cp/job_{job_id}/task_logs/job_{job_id}_task_{task_id}_stderr.txt
                )
            )
            ...
            # same thing between stderr and stdout
            std_err_and_out = [
                OutputFile(
            ...
            task = TaskAddParameter(
                ...
                container_settings=TaskContainerSettings(
                    ...
                    # container_run_options="--gpus all", # For GPU running
                    container_run_options=f"--shm-size={api_config.SHARED_MEMORY_SIZE}g"  # Adjust the shared memory size as needed
                ),
    ...
    def get_failed_tasks(self, job_id:str):
        tasks = self.batch_client.task.list(job_id,
                                            task_list_options=TaskListOptions(
                                                filter='state eq \'completed\'',
                                                select='id, executionInfo'  # only the id field will be non-empty

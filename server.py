...
@app.route(f'/')
def hello():
    return f'AWC Camera traps batch processing API'


@app.route(f'/request_detections', methods=['POST'])
def request_detections():
    ...
    if caller_id is None or caller_id not in app_config.get_allowlist(): # only 'awc' is allowed
        msg = ('Parameter caller is not supplied or is not on our allowlist. '
               'Please email it.support@australianwildlife.org to request access.')
        return make_error(401, msg)

    use_url = post_body.get('use_url', False) # always False
    ...
    try:
        num_active_jobs = batch_job_manager.get_num_active_jobs() # 1?
        if num_active_jobs >= api_config.MAX_BATCH_ACCOUNT_ACTIVE_JOBS: # 300
...
@app.route(f'/cancel_request', methods=['POST'])
def cancel_request():
    """
...
def retrieve_job_status(job_id: str):
    ...
    time_passed = datetime.now(timezone.utc) - last_updated.replace(tzinfo=timezone.utc)
...
@app.route(f'/default_model_version')
def get_default_model_version() -> str:
    return api_config.DEFAULT_MD_VERSION # 5a


@app.route(f'/supported_model_versions')
